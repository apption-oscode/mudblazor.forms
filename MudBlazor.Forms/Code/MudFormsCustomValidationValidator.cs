...
public class MudFormsCustomValidationValidator<T> : ComponentBase
{
    [CascadingParameter]
    ...
    [Inject] private ILogger<MudFormsCustomValidationValidator<T>>? Logger { get; set; }
    ...
    protected override void OnInitialized()
    {
        ...
        if (EditContext is null || ModelFormContext is null)
            throw new NullReferenceException(
                $"{nameof(MudFormsCustomValidationValidator<T>)} must be placed within an {nameof(MudModelForm<T>)}");

        if (Model is null)
            throw new NullReferenceException(
                $"Model must be set on {nameof(MudFormsCustomValidationValidator<T>)}");
